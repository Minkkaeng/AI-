    ...
    <div className="flex flex-col w-full h-full bg-slate-50 font-sans text-neoul-heuk">
      ...
      <header className="h-20 bg-white border-b border-black/5 flex items-center justify-between px-10 flex-shrink-0 z-50">
        ...
        <div className="flex items-center gap-4">
           ...
           <button className="p-2.5 bg-neoul-heuk rounded-xl text-white font-bold px-6">MY LOG</button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[450px] bg-white border-r border-black/5 flex flex-col z-40 shadow-2xl shadow-black/5 overflow-y-auto custom-scrollbar p-8">
           <h3 className="text-2xl font-bold tracking-tight mb-8">한국의 유산을 찾아서</h3>
        ...
        {/* Interactive Precise Map */}
        <main className="flex-1 relative bg-white overflow-hidden p-12">
           <div className="absolute inset-0 flex items-center justify-center">
              <img src="map-bg.png" className="max-w-full h-[90%] object-contain opacity-100" alt="South Korea Map" />
              
              {/* Landmark 1: Seoul */}
              <motion.div 
                whileHover={{ scale: 1.1, zIndex: 30 }}
                className="absolute top-[30%] left-[42%] cursor-pointer group"
              ...
              >
                <div className="bg-white p-2 rounded-3xl shadow-2xl border border-black/5 flex flex-col items-center">
                  <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-100 group-hover:ring-4 ring-neoul-mint transition-all">
              ...
              <motion.div 
                whileHover={{ scale: 1.1, zIndex: 30 }}
                className="absolute top-[58%] left-[40%] cursor-pointer group"
                onClick={() => setSelectedPlace(MOCK_PLACES[1])}
              >
                <div className="bg-white p-2 rounded-3xl shadow-2xl border border-black/5 flex flex-col items-center">
                  <div className="w-20 h-20 rounded-2xl overflow-hidden bg-slate-100 group-hover:ring-4 ring-neoul-jeok transition-all">
              ...
              <motion.div 
                whileHover={{ scale: 1.1, zIndex: 30 }}
                className="absolute bottom-[10%] left-[38%] cursor-pointer group"
              >
                <div className="bg-white p-3 rounded-full shadow-2xl border border-black/5 flex flex-col items-center">
                   <span className="text-3xl">🌴</span>
                   <span className="text-[10px] font-black mt-1 uppercase">JEJU</span>
                </div>
              </motion.div>

              {/* Airport */}
              <div className="absolute top-[28%] left-[36%] z-10">
                 <div className="bg-neoul-cheong p-3 rounded-full shadow-2xl border-4 border-white flex items-center justify-center">
           ...
           <AnimatePresence>
             {selectedPlace && (
               ...
               >
                  ...
                  <div className="w-full aspect-video rounded-[3rem] overflow-hidden mb-10 shadow-2xl"><img src={selectedPlace.id === 1 ? 'gyeongbokgung.png' : 'hanok.png'} className="w-full h-full object-cover" /></div>
                  <h3 className="text-4xl font-black mb-6">{selectedPlace.name}</h3>
                  <p className="text-base font-medium leading-relaxed text-neoul-heuk/60 mb-10">{selectedPlace.description}</p>
                  <button className="w-full bg-neoul-heuk text-white py-6 rounded-[2rem] text-lg font-black flex items-center justify-center gap-3">
                     <Play className="w-6 h-6 fill-white" /> AI 도슨트 시작
                  </button>
               </motion.div>
             )}
           </AnimatePresence>
